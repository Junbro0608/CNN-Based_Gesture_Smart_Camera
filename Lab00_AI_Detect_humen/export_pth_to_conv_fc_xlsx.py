...
MODEL_PATH = Path("./save_pt/best_model_master128.pth")
CONV_OUT_XLSX = Path("./save_pt/int8_weights_conv_fomat.xlsx")
FC_OUT_XLSX = Path("./save_pt/int8_weights_fc_reordered.xlsx")
PACKED_BE64_XLSX = Path("./save_pt/int8_weights_conv_fc_be64.xlsx")
COMBINED_BE64_MEM = Path("./save_pt/int8_weights_conv_fc_be64.mem")
...
def quantize_int32_bias_fixed127(tensor: torch.Tensor):
    arr = tensor.detach().cpu().numpy().astype(np.float32)
    q = np.round(arr * 127.0).astype(np.int32)
...
def build_bn_folded_conv_bias(state_dict, layer_name: str, out_c: int):
    ...
    b_fold = beta + (b_conv - mean) * gamma / sqrt(var + eps)
    """
...
def build_conv_format_rows(state_dict):
    ...
    for layer_name in conv_layers:
        ...
        for out_base in range(0, out_c, 8):
            ...
            # Insert 32-bit bias values right after each 8-channel block's K22 sequence.
        ...
        packed_rows.append(
            {
                ...
                "Byte1": f"0x{word_bytes[1]:02X}",
                "Byte2": f"0x{word_bytes[2]:02X}",
                "Byte3": f"0x{word_bytes[3]:02X}",
