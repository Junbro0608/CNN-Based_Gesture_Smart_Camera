...
module tb_CNN_accelerator_mem_all;
	...
	localparam bit ENABLE_FILE_LOG = 1'b1;
	...
	task automatic run_case(input string mem_path, input logic expected_result);
		...
		begin
			...
			if (ENABLE_CONSOLE_LOG) $display("[TB] running summary: pass=%0d fail=%0d", pass_count, fail_count);
			if (ENABLE_FILE_LOG && (log_fd != 0))
				$fdisplay(log_fd, "[TB] running summary: pass=%0d fail=%0d", pass_count, fail_count);
	...
	task automatic run_all_cases;
		...
		string mem_path;
		begin
			for (i = 0; i < PERSON_COUNT; i = i + 1) begin
				mem_path = $sformatf("%s/person%0d.mem", MEM_BASE_DIR, i);
				run_case(mem_path, 1'b1);
			end

			for (i = 0; i < NONPERSON_COUNT; i = i + 1) begin
				mem_path = $sformatf("%s/nonperson%0d.mem", MEM_BASE_DIR, i);
				run_case(mem_path, 1'b0);
			end
		end
	endtask
	...
	initial begin
		...
		if (ENABLE_FILE_LOG) begin
			log_fd = $fopen("tb_CNN_accelerator_mem_all.log", "w");
			fail_log_fd = $fopen("tb_CNN_accelerator_mem_all_fail_images.log", "w");
		...
		if (err_count == 0) begin
			...
			if (ENABLE_FILE_LOG && (log_fd != 0))
				$fdisplay(log_fd, "ALL TESTS PASSED (%0d cases)", TOTAL_CASES);
		end else begin
