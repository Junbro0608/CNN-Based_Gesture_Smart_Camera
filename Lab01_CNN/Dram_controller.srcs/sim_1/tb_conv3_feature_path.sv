...
module tb_conv3_feature_path;
    ...
    padding #(
        ...
        .clk            (clk),
        .rst_n          (rst_n),
        .padding_size   ((dut.layer_index == 0) ? 8'd128 :
                         (dut.layer_index == 1) ? 8'd64 : 8'd32),
    ...
    always @(posedge clk) begin
        ...
        if (dut.layer_index == 1)
            padding_data_rdata <= buffer_b[delayed_data_raddr];
        else
            padding_data_rdata <= buffer_a[delayed_data_raddr];

        if (weight_addr < (dut.layer_index == 0 ? 9 : 72)) begin
            case (dut.layer_index)
                0: weight_rdata <= {8{8'sd1}};
                1: weight_rdata <= {8{8'sd4}};
                default: weight_rdata <= {8{8'sd10}};
            endcase
        end else begin
            weight_rdata <= '0;
        end
    ...
    task automatic wait_done;
        begin
            cycle_count = 0;
            while ((done !== 1'b1) && (cycle_count < 5_000_000)) begin
                @(posedge clk);
    ...
    initial begin
        ...
        if (conv1_write_count != EXPECTED_CONV1_WRITES) begin
            error_count = error_count + 1;
            $display("[FAIL] Conv1 writes=%0d exp=%0d", conv1_write_count, EXPECTED_CONV1_WRITES);
        end
        if (conv2_write_count != EXPECTED_CONV2_WRITES) begin
            error_count = error_count + 1;
            $display("[FAIL] Conv2 writes=%0d exp=%0d", conv2_write_count, EXPECTED_CONV2_WRITES);
        end
        if (conv3_write_count != EXPECTED_CONV3_WRITES) begin
            error_count = error_count + 1;
            $display("[FAIL] Conv3 writes=%0d exp=%0d", conv3_write_count, EXPECTED_CONV3_WRITES);
        end
