                    ...
                    if (weight_data_pending) begin
                        for (ch = 0; ch < NUM_CH; ch = ch + 1) begin
                            if (load_prefetch_bank)
                                prefetched_weight_out[ch][capture_word_index] <=
                                    $signed(wt_rdata[ch*8 +: 8]);
                            else
                                active_weight_out[ch][capture_word_index] <=
                                    $signed(wt_rdata[ch*8 +: 8]);
                        end
                ...
                LOAD_BIAS: begin
                    if (bias_data_pending) begin
                        if (load_prefetch_bank) begin
                            prefetched_bias_out[bias_capture_word_index * 2] <=
                                $signed(wt_rdata[31:0]);
                            prefetched_bias_out[bias_capture_word_index * 2 + 1] <=
                                $signed(wt_rdata[63:32]);
                        end else begin
                            active_bias_out[bias_capture_word_index * 2] <=
                                $signed(wt_rdata[31:0]);
                            active_bias_out[bias_capture_word_index * 2 + 1] <=
                                $signed(wt_rdata[63:32]);
                        end
                    end
