...
module requantize_pipeline #(
    ...
    parameter [4:0] SHIFT_VAL_L4 = 5'd20,
    parameter [4:0] SHIFT_VAL_L5 = 5'd20,
    parameter signed [31:0] MULT_FACTOR_L1 = 32'sd3275,
    parameter signed [31:0] MULT_FACTOR_L2 = 32'sd2809,
    parameter signed [31:0] MULT_FACTOR_L3 = 32'sd3543,
    parameter signed [31:0] MULT_FACTOR_L4 = 32'sd4128,
    parameter signed [31:0] MULT_FACTOR_L5 = 32'sd1651,
    parameter signed [7:0] ZERO_POINT_L1 = 8'sd0,
    parameter signed [7:0] ZERO_POINT_L2 = 8'sd0,
    parameter signed [7:0] ZERO_POINT_L3 = 8'sd0,
    parameter signed [7:0] ZERO_POINT_L4 = 8'sd0,
    ...
    always_comb begin
        ...
        case (layer)
            ...
            3'd1: begin cfg_shift_val = SHIFT_VAL_L1; cfg_mult_factor = MULT_FACTOR_L1; cfg_zero_point = ZERO_POINT_L1; end
            3'd2: begin cfg_shift_val = SHIFT_VAL_L2; cfg_mult_factor = MULT_FACTOR_L2; cfg_zero_point = ZERO_POINT_L2; end
            3'd3: begin cfg_shift_val = SHIFT_VAL_L3; cfg_mult_factor = MULT_FACTOR_L3; cfg_zero_point = ZERO_POINT_L3; end
            3'd4: begin cfg_shift_val = SHIFT_VAL_L4; cfg_mult_factor = MULT_FACTOR_L4; cfg_zero_point = ZERO_POINT_L4; end
            3'd5: begin cfg_shift_val = SHIFT_VAL_L5; cfg_mult_factor = MULT_FACTOR_L5; cfg_zero_point = ZERO_POINT_L5; end
            default: begin end
        endcase
    end
    ...
    always_comb begin
        ...
        if (bypass_stage3)
            quantized_next = bypass_data_stage3[7:0];
        else if (quantized_acc > 64'sd127)
            quantized_next = 8'sd127;
        else if (quantized_acc < -64'sd128)
            quantized_next = -8'sd128;
        else
            quantized_next = quantized_acc[7:0];
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (valid_stage3)
                quant_stage4 <= quantized_next;

            out_valid <= valid_stage4;
            if (valid_stage4)
                quant_out <= quant_stage4;
        end
    end
