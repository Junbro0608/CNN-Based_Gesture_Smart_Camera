...
module tb_fc;
    ...
    localparam integer GROUP_STRIDE       = INPUT_COUNT + 4;
    localparam integer WT_WORD_COUNT      = GROUP_COUNT * GROUP_STRIDE;
    localparam integer TB_WT_DEPTH        = WT_WORD_COUNT;
    localparam integer TB_DATA_DEPTH      = INPUT_COUNT;
    ...
    integer previous_weight_address;
    integer weight_address_seen;
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            done_count       = 0;
            last_write_cycle = -1;
            previous_weight_address = 0;
            weight_address_seen = 0;
        end
        else begin
            if ((^wt_raddr === 1'bx) ||
                (^wt_rdata === 1'bx) ||
            ...
            // 유효 read 사이의 대기 cycle에는 주소 유지가 가능하지만,
            // 주소가 바뀔 때는 반드시 이전 주소보다 정확히 1만 증가해야 한다.
            if (weight_address_seen != 0) begin
                if ((wt_raddr != previous_weight_address) &&
                    (wt_raddr != (previous_weight_address + 1))) begin
                    $fatal(
                        1,
                        "Weight 주소 비순차 변화: previous=%0d current=%0d",
                        previous_weight_address,
                        wt_raddr
                    );
                end
            end
            previous_weight_address = wt_raddr;
            weight_address_seen = 1;
    ...
    task automatic build_test_vectors;
        ...
        begin
            ...
            for (output_index = 0;
                ...
                // 1024개 signed MAC 결과와 Bias를 모두 반영해 golden 값을 만든다.
            ...
            // Group마다 Weight 1024 word 뒤에 Bias 4 word를 순서대로 packing한다.
            for (group_index = 0;
                ...
                for (input_index = 0;
                     input_index < INPUT_COUNT;
                     input_index = input_index + 1) begin
                    weight_memory[group_base + input_index] =
                        pack_weight_eight(
    ...
    initial begin
        ...
        rst_n         = 1'b0;
        start         = 1'b0;
        input_length  = 10'd1023;
        output_length = 10'd15;
        finish_en     = 1'b0;
        cycle_count   = 0;
        ...
        $display("==================================================");
        $display("PASS: FC 1024-input / 16-output self-checking test");
