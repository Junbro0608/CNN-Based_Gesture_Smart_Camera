...
module fc_memory_adapter #(
    ...
    input logic [$clog2(WT_ADDR_WIDTH)-1:0] core_weight_read_offset,
    input logic [$clog2(DATA_ADDR_WIDTH)-1:0] core_data_read_offset,
    input logic core_data_write_en,
    input logic [$clog2(DATA_ADDR_WIDTH)-1:0] core_data_write_offset,
    input logic [DATA_DATA_WIDTH-1:0] core_data_write_data,

    output logic [  WT_DATA_WIDTH-1:0] core_weight_read_data,
    output logic [DATA_DATA_WIDTH-1:0] core_data_read_data,
    ...
    // 외부 asynchronous read data는 register나 valid 생성 없이 직접 전달한다.
