...
module conv #(
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            conv_write_we_reg   <= conv_quant_valid && conv_meta_valid_stage1;
            conv_write_bank_reg <= conv_meta_bank_stage1;
            conv_write_addr_reg <= conv_meta_addr_stage1;
            conv_write_data_reg <= conv_wdata_quantized;
            packed_write_we_reg <=
                packed_quant_valid && conv_meta_valid_stage1;
            if (packed_quant_valid && conv_meta_valid_stage1)
    ...
    requantize_no_mult U_REQUANTIZE_CONV_WRITE (
        .clk     (clk),
        .rst_n   (rst_n),
    ...
    generate
        ...
        for (packed_quant_ch = 0; packed_quant_ch < NUM_CH;
            ...
            requantize_no_mult U_REQUANTIZE_PACKED_WRITE (
                .clk     (clk),
                .rst_n   (rst_n),
                .in_valid(controller_data_write_enable),
                .layer   (requantize_layer),
