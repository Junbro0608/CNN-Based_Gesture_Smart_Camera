...
train_transform = transforms.Compose([
    ...
    transforms.RandomRotation(10),                  # ±10도 회전 (완화)
    transforms.RandomAffine(0, translate=(0.05, 0.05)),  # 이동 변환 (완화)
    transforms.ToTensor(),
...
class PersonClassifierCNN(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),   # 126 -> 63

            nn.Conv2d(16, 32, kernel_size=3),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),   # 61 -> 30
            nn.Dropout2d(0.1),
            ...
            nn.Conv2d(64, 128, kernel_size=3),  # 채널 확장: 특징 추출 강화
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        # 분류기 (Classifier) - Sigmoid 제거: BCEWithLogitsLoss 내부에서 처리
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=8, factor=0.5, min_lr=1e-5) # Val Loss 기반 LR 감소
...
plt.subplot(1, 2, 2)
plt.plot(range(1, epochs + 1), train_acc_history, marker='o', color='orange', label='Train Acc')
plt.plot(range(1, epochs + 1), val_acc_history, marker='o', color='green', label='Val Acc')
