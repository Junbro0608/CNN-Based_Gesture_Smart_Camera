...
void PrintGpio0Debug(void) {
    uint32_t gpio_value = Xil_In32(GPIO0_BASE_ADDR);
    uint32_t low_1bit = gpio_value & 0x1;
    int8_t upper_8bit_s = (int8_t)((gpio_value >> 1) & 0xFF);

    xil_printf("GPIO_0 raw=0x%08X | low1=%lu | upper8_s=%d (0x%02X)\r\n",
               (unsigned int)gpio_value,
               (unsigned long)low_1bit,
               (int)upper_8bit_s,
               (unsigned int)((uint8_t)upper_8bit_s));
}
...
int main() {
    ...
    xil_printf("GPIO debug stream start (0.5s period)\r\n");
    while (1) {
        PrintGpio0Debug();
        DelayHalfSecond();
    }
