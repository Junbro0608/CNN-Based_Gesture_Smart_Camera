...
module gray_128x128 #(
    ...
    //--------------------------------------------------------------
    // 캡처 (수정)
    //  - sof 사이클의 첫 픽셀도 mem[0]에 기록 (기존엔 유실됨)
    //  - 마지막 픽셀이 프레임 안에서 16383번지에 써지도록 정렬
    //--------------------------------------------------------------
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            if (sof) begin
                if (!busy) begin
                    capturing <= 1'b1;
                    wr_addr   <= 14'd1;  // 0번지는 이번 사이클에 이미 씀
                end else begin
                    capturing     <= 1'b0;
                    frame_dropped <= 1'b1;
                end
    ...
    //--------------------------------------------------------------
    // 프레임 완료 → start 트리거 (수정)
    //  - last_pix가 프레임 마지막 픽셀 사이클에 정확히 발사됨
    //  - last_pix_reg 클리어를 start 발행 시점으로 분리 (무한 재시작 방지)
    //--------------------------------------------------------------
    ...
    // CNN 전용 읽기 포트: padding/conv 파이프라인은 주소 후 1-cycle에
    // 데이터가 나오는 동기 BRAM을 가정하므로 여기서 레지스터 한 단만 거친다.
    always @(posedge aclk) begin
