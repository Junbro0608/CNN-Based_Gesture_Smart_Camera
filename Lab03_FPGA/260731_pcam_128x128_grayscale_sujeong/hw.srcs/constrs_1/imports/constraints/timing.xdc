...
# Override demo input delays for current 420 Mbps/lane setting.
# Original demo values are for ~200 Mbps/lane and are too loose/tight for this build.
# Remove previously applied constraints on the same ports to avoid stacking effects.
reset_input_delay [get_ports {dphy_data_hs_n[*] dphy_data_hs_p[*]}]

# UI at 420 Mbps is ~2.381 ns. Use 0.15UI/0.85UI capture window.
set_input_delay -clock [get_clocks dphy_hs_clock_p] -clock_fall -min -add_delay 0.357 [get_ports {dphy_data_hs_n[*] dphy_data_hs_p[*]}]
set_input_delay -clock [get_clocks dphy_hs_clock_p] -clock_fall -max -add_delay 2.024 [get_ports {dphy_data_hs_n[*] dphy_data_hs_p[*]}]
set_input_delay -clock [get_clocks dphy_hs_clock_p] -min -add_delay 0.357 [get_ports {dphy_data_hs_n[*] dphy_data_hs_p[*]}]
set_input_delay -clock [get_clocks dphy_hs_clock_p] -max -add_delay 2.024 [get_ports {dphy_data_hs_n[*] dphy_data_hs_p[*]}]
