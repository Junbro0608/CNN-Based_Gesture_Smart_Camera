...
module CH #(
    ...
    // unsigned 8-bit Pixel을 signed 9-bit 양수로 변환한 값
    ...
    always_comb begin
        ...
        // signed 8-bit Bias를 signed 32-bit로 부호 확장
        bias_extended = $signed({{24{bias_in[7]}}, bias_in});

        for (i = 0; i < 9; i = i + 1) begin
            // unsigned 8-bit Pixel에 0을 붙여
            // signed 9-bit 양수로 변환
        ...
        if (!rst_n) begin
            ...
            if (acc_clear) begin
                ...
                if (first_ic && last_ic) begin
                    ...
                    full_result  <= conv_sum + bias_extended;

                    // 32-bit 결과를 OUTPUT_SHIFT만큼
                    // 산술 오른쪽 Shift한 후 8-bit 출력에 저장
                    result_out   <= (conv_sum + bias_extended) >>> OUTPUT_SHIFT;
