...
// Reads one 3x3-weight set and one bias word from an external asynchronous
...
// follow wt_raddr combinationally, as in:
//
...
module Weight_Loader #(
    ...
    // External asynchronous-read 64-bit weight-buffer interface.
    output logic                    wt_ren,
    ...
    always_comb begin
        ...
        case (state)
            READ_WEIGHT:
                wt_raddr = weight_start_addr + word_index;
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                READ_WEIGHT: begin
                    ...
                    if (word_index == NUM_KERNEL_WORDS-1) begin
                        state <= READ_BIAS;
                    end else begin
